...
void GraphicsApiManager::init(AppInfo* appInfo, WindowManager* windowManager) {
    ...
    this->createSwapChain(windowManager->window);
}
...
void GraphicsApiManager::pickPhysicalDevice() {
    ...
    for (const auto& device : devices) {
        if (this->isDeviceSuitable(device)) {
            // possible device selection here!
            this->physicalDevice = device;

            // get properties of the device, set to class variable
    ...
    std::string deviceNameMessage = std::format("Physical device selected: {0}", this->physicalDeviceProperties.deviceName);
    logMessage(deviceNameMessage.c_str());
}

void GraphicsApiManager::createSwapChain(GLFWwindow* window) {
...
bool GraphicsApiManager::isDeviceSuitable(VkPhysicalDevice device) {
...
QueueFamilyIndices GraphicsApiManager::findQueueFamilies(VkPhysicalDevice device) {
...
SwapChainSupportDetails GraphicsApiManager::querySwapChainSupport(VkPhysicalDevice device) {
...
VkSurfaceFormatKHR GraphicsApiManager::chooseSwapSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& availableFormats) {
    // possible change this to a different format to generate unique effects? raytracing with a lesser color space could be cool
...
VkPresentModeKHR GraphicsApiManager::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
    ...
    // non v-sync, for sean, as a treat
    return VK_PRESENT_MODE_FIFO_KHR;
}

// resolution stuff
VkExtent2D GraphicsApiManager::chooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities, GLFWwindow* window) {
